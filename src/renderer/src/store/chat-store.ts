...
import { wrapWithAppContext } from '@shared/app-settings'
import type {
  AppRoute,
  ChatState,
  InitialSetupMode,
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  sendMessage: async (text, mode, overrides) => {
    ...
    if (get().busy || hasPendingActiveTurn) {
      ...
      const composerModel =
        overrideModel ?? get().composerModel.trim()
      const userModelChip =
    ...
    try {
      ...
      const baseText = wrapWithAppContext(trimmedText)
      let runtimeText = buildPromptWithAttachments(baseText, attachments)
