...
const DEV_PREVIEW_ASSISTANT_ACTION_RE =
  /\b(?:open|visit|browse|view|check(?:\s+it)?\s+out|go\s+to)\b/i
const DEV_PREVIEW_ASSISTANT_STATUS_RE =
  /\b(?:served|serving|running|started|available|reachable|live\s+at|running\s+at|available\s+at|listening\s+on|app\s+running\s+at|serving\s+at)\b/i
