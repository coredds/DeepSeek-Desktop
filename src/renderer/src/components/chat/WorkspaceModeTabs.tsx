...
export function WorkspaceModeTabs({
  ...
  return (
    <div className="mb-4 rounded-[12px] border border-ds-border-muted/45 bg-ds-subtle/72 p-0.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.52)] backdrop-blur dark:bg-white/[0.045] dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]">
      <button
        type="button"
        onClick={onPureChatOpen}
        className={`${tabClass(activeView === 'chat-pure')} w-full h-[34px] px-3`}
      >
        <MessageSquare className="h-3.5 w-3.5 shrink-0" strokeWidth={1.9} />
        <span className="truncate">{t('chat')}</span>
      </button>

      <div className="mx-1 my-1 border-t border-ds-border-muted/25" />
