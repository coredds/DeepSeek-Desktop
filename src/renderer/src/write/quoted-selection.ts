...
export const WRITE_QUOTE_ORIGINAL_START = '[Quoted Original]'
export const WRITE_QUOTE_ORIGINAL_END = '[/Quoted Original]'
export const WRITE_CONTEXT_HEADING = '[Writing Context]'
export const WRITE_QUOTE_HEADING = '[Quoted Snippet]'
...
export function formatWriteQuotedSelectionForPrompt(selection: WriteQuotedSelection): string {
  if (selection.lineStart != null && selection.lineEnd != null) {
    return [
      `[Quoted Snippet] ${selection.sourceTitle} (lines ${selection.lineStart}-${selection.lineEnd}, ${selection.charCount} chars) path: ${selection.sourceFilePath}`,
  ...
  return [
    `[Quoted Snippet] ${selection.sourceTitle} (${selection.charCount} chars) path: ${selection.sourceFilePath}`,
    WRITE_QUOTE_ORIGINAL_START,
    selection.text,
    WRITE_QUOTE_ORIGINAL_END
  ].join('\n')
...
export function composeWritePrompt(
  ...
  if (context.workspaceRoot?.trim()) {
    contextLines.push(`Workspace: ${context.workspaceRoot.trim()}`)
  }
  if (context.activeFilePath?.trim()) {
    contextLines.push(`Active file: ${relativeWritePath(context.workspaceRoot ?? '', context.activeFilePath)}`)
  }
...
function parseContextBlock(text: string): WritePromptDisplayContext {
  ...
  for (const line of lines) {
    ...
    const fileMatch = line.match(/^Active file:\s*(.+)$/)
    if (fileMatch?.[1]) {
...
function parseQuoteHeader(header: string): Omit<WritePromptDisplayQuote, 'text'> {
  ...
  const pathSplit = body.match(/^(.*?)\s*path:\s*(.+)$/)
  const titleAndMeta = (pathSplit?.[1] ?? body).trim()
  const sourceFilePath = pathSplit?.[2]?.trim()
  const metaMatch = titleAndMeta.match(/^(.*?)\s*\((?:(?:lines?\s*)?(\d+)[-–—](\d+),\s*)?(\d+)\s*chars\)$/)
