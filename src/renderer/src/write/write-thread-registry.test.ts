...
describe('write-thread-registry', () => {
  ...
  it('hydrates leaked write assistant threads from configured write workspaces', () => {
    const leaked = {
      ...thread('write-thread', '/Users/zxy/.deepseekdesktop/write_workspace'),
      title: WRITE_ASSISTANT_THREAD_TITLE
    }
    const normalCodeThread = {
      ...thread('code-thread', '/Users/zxy/.deepseekdesktop/write_workspace'),
    ...
    const registry = hydrateWriteThreadRegistry(
      ...
      ['/Users/zxy/.deepseekdesktop/write_workspace'],
      emptyWriteThreadRegistry()
    )
  ...
  it('hydrates legacy tilde write assistant threads under the configured absolute workspace', () => {
    const legacyThread = {
      ...thread('legacy-write-thread', '~/.deepseekdesktop/write_workspace'),
      title: WRITE_ASSISTANT_THREAD_TITLE
    }
    ...
    expect(registry.workspaces['/Users/zxy/.deepseekdesktop/write_workspace'].threadIds).toEqual([
      'legacy-write-thread'
    ])
    expect(
      activeWriteThreadForWorkspace(
