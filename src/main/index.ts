...
const APP_USER_MODEL_ID = 'com.coredds.deepseekdesktop'
const startupTraceEnabled = process.env.DEEPSEEK_DESKTOP_STARTUP_TRACE === '1'
...
async function showTurnCompleteNotification(
  ...
  const title = normalizeNotificationText(payload.title, 'DeepSeek Desktop', 80)
  ...
  try {
    ...
    const nested =
      parsed.error && typeof parsed.error === 'object'
        ? (parsed.error as Record<string, unknown>).message?.toString().trim() ?? ''
        : ''
    const topLevel =
...
async function ensureRuntimeOnce(settings: AppSettingsV1): Promise<void> {
    ...
    if (threadApi.ok) {
      if (!isDeepseekChildRunning() && settings.deepseek.autoStart && hasApiKey) {
        ...
        if (launch.state === 'deepseek' && !launch.matches) {
          console.warn(
            `[deepseek-desktop] restarting runtime on port ${settings.deepseek.port}; launch config mismatch: ${launch.reason}`
          )
  ...
  try {
    ...
    console.error('[deepseek-desktop] failed to start deepseek:', e)
    throw e
  }
...
function createWindow(): void {
  ...
  mainWindow.webContents.on('preload-error', (_event, preloadPath, error) => {
    ...
    console.error(`[deepseek-desktop] failed to load preload ${preloadPath}:`, error)
...
async function restartManagedRuntimeForSettingsChange(
  ...
  if (samePort) {
    ...
    if (!reclaimed.ok) {
      console.warn('[deepseek-desktop] runtime restart skipped:', reclaimed.message)
      return
    }
  }
  ...
  try {
    ...
    if (!healthy) {
      console.warn('[deepseek-desktop] runtime restart did not become healthy after settings change')
    }
  } catch (e) {
    console.warn('[deepseek-desktop] runtime restart failed after settings change:', e)
  }
}
...
app.whenReady().then(async () => {
    ...
    if (isDeepseekChildRunning()) {
      ...
      if (resolveConfiguredApiKey(settings) && settings.deepseek.autoStart) {
        try {
          ...
          console.warn('[deepseek-desktop] runtime restart failed after deepseek-tui update:', error)
        }
      }
    }
  ...
  ipcMain.handle('deepseek:spawn-if-needed', async () => {
    ...
    try {
      ...
      console.error('[deepseek-desktop] spawn:', e)
  ...
  const message = error instanceof Error ? error.message : String(error)
  console.error('[deepseek-desktop] startup failed:', error)
