...
function logFileName(timestamp: Date): string {
  ...
  return `deepseek-desktop-${timestamp.getFullYear()}-${pad(timestamp.getMonth() + 1)}-${pad(timestamp.getDate())}.log`
}
...
async function pruneOldLogs(): Promise<void> {
  try {
    ...
    for (const entry of entries) {
      if (!entry.startsWith('deepseek-desktop-') || !entry.endsWith('.log')) continue
      try {
