...
function formatFeishuMirrorText(text: string, direction: 'user' | 'assistant'): { markdown: string } {
  ...
  if (direction === 'user') {
    return {
      markdown: `**From DeepSeek Desktop**\n\n> ${trimmed.replace(/\n/g, '\n> ')}`
    }
  }
  ...
  private async handleWebhook(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      if (im.secret) {
        ...
        const headerSecret = Array.isArray(req.headers['x-deepseek-desktop-secret'])
          ? req.headers['x-deepseek-desktop-secret'][0]
          : req.headers['x-deepseek-desktop-secret']
