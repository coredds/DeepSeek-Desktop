...
export async function runClawScheduleMcpServerFromArgv(argv: string[]): Promise<boolean> {
  ...
  const server = new McpServer(
    { name: 'deepseek-desktop-schedule', version: '0.1.0' },
    { capabilities: { logging: {} } }
  )

  const registerListTool = (name: string): void => {
    server.registerTool(name, {
      description: 'List scheduled tasks managed by the currently running DeepSeek Desktop app.'
    }, async () => {
      try {
  ...
  const registerCreateTool = (name: string): void => {
    server.registerTool(name, {
      description: 'Create a scheduled task in DeepSeek Desktop. Supports one-time (`at`), daily, or interval schedules.',
      inputSchema: {
  ...
  const registerUpdateTool = (name: string): void => {
    server.registerTool(name, {
      description: 'Update an existing DeepSeek Desktop scheduled task.',
      inputSchema: {
  ...
  const registerDeleteTool = (name: string): void => {
    server.registerTool(name, {
      description: 'Delete a scheduled task from DeepSeek Desktop.',
      inputSchema: {
        task_id: z.string().min(1).describe('Task id returned by gui_schedule_list or gui_schedule_create')
      }
    }, async ({ task_id }) => {
