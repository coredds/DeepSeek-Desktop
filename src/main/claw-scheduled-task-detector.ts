...
const SCHEDULED_TASK_CANDIDATE_RE =
  /(?:remind|reminder|alarm|timer|schedule|scheduled|notify|notification|later|tomorrow|tonight|in\s+\d+\s+(?:seconds?|minutes?|hours?|days?|weeks?))/iu
...
function normalizeReminderBody(value: string): string {
  return value
    ...
    .replace(/^[,:\s]+/u, '')
    .replace(/[.!?~:;\s]+$/u, '')
    .replace(/^(?:a bit|a moment|briefly)\s*/i, '')
    .trim()
}
...
function normalizeReminderName(value: string): string {
  ...
  return /reminder$/iu.test(compact) ? compact : `${compact} reminder`
}
