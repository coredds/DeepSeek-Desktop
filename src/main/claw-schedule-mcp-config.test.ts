...
function createSettings(patch: Partial<AppSettingsV1['claw']['im']> = {}): AppSettingsV1 {
  ...
  return {
    ...
    claw: { ...claw, im: { ...claw.im, ...patch } }
  }
}

const launch: ClawScheduleMcpLaunchConfig = {
  appPath: '/Applications/DeepSeek Desktop.app',
  execPath: '/Applications/DeepSeek Desktop.app/Contents/MacOS/DeepSeek Desktop',
  isPackaged: false
}
  ...
  it('removes legacy config.toml claw_schedule blocks without touching other MCP servers', () => {
    const cleaned = removeLegacyClawScheduleTomlConfig(
      [
        ...
        'args = []',
        '',
        '# DeepSeek Desktop plugin:mcp:claw-schedule START',
        '[mcp_servers.claw_schedule]',
        'command = "electron"',
        'args = []',
        '# DeepSeek Desktop plugin:mcp:claw-schedule END',
        '',
        '[providers.deepseek]',
        'api_key = ""'
      ].join('\n')
    ...
    expect(cleaned).not.toContain('DeepSeek Desktop plugin:mcp:claw-schedule')
  })
