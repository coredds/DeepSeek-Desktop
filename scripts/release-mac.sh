...
#   DEEPSEEK_DESKTOP_RUNTIME_CACHE=0    disable bundled runtime cache
#
# After this completes, run on Windows (same version):
#   ./scripts/release-win.sh --tag v<RELEASE_VERSION from output> --publish
...
build_mac_arch() {
  ...
  DEEPSEEK_DESKTOP_DIST_DIR="${output_dir}" \
...
copy_mac_arch_artifacts() {
  ...
  shopt -s nullglob
  files=("${output_dir}"/DeepSeek-Desktop-*-mac-"${arch}".*)
  shopt -u nullglob
...
collect "macOS arm64 dmg" "dist/DeepSeek-Desktop-*-mac-arm64.dmg"
collect "macOS x64 dmg" "dist/DeepSeek-Desktop-*-mac-x64.dmg"
collect "macOS arm64 zip" "dist/DeepSeek-Desktop-*-mac-arm64.zip"
collect "macOS x64 zip" "dist/DeepSeek-Desktop-*-mac-x64.zip"
collect "macOS blockmap" "dist/DeepSeek-Desktop-*-mac-*.zip.blockmap"
...
cyan "  https://github.com/coredds/DeepSeek-Desktop/releases/tag/${TAG_NAME}"
