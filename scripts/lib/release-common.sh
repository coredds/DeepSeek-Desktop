...
release_load_local_env() {
  local env_file="${DEEPSEEK_DESKTOP_RELEASE_ENV:-}"
...
release_compute_version() {
  ...
  release_validate_semver "${BASE_VERSION}" || die "package.json version must be a valid semver, got: ${BASE_VERSION}"

  if [[ "${RELEASE_BUMP:-}" == "none" && -n "${RELEASE_TAG:-}" ]]; then
    TAG_NAME="${RELEASE_TAG}"
    [[ "${TAG_NAME}" == v* ]] || TAG_NAME="v${TAG_NAME}"
    RELEASE_VERSION="${TAG_NAME#v}"
    release_validate_semver "${RELEASE_VERSION}" || die "Release tag must be vX.Y.Z: ${TAG_NAME}"
    RELEASE_NAME="DeepSeek Desktop ${RELEASE_VERSION}"
    LATEST_TAG=""
    return
  fi
...
release_clean_dist_artifacts() {
  ...
  rm -f "${ROOT}"/dist/DeepSeek-Desktop-* "${ROOT}"/dist/DeepSeek\ GUI-* "${ROOT}"/dist/latest*.yml "${ROOT}"/dist/*.blockmap
}
...
release_write_notes_file() {
...

  {
    ...
    if [[ "${SIGNING:-false}" == true ]]; then
      echo "- macOS: ✅ Developer ID signed + notarized"
    else
      echo "- macOS: Unsigned build"
    fi
    echo "- Platform: macOS (arm64 + Intel x64) · Windows (\`release-win.ps1\` / \`release-win.sh\`)"
  } >>"${dest}"
}
