...
function Assert-Semver([string]$Version) {
  if ($Version -notmatch '^\d+\.\d+\.\d+$') {
    Write-Err "Release tag must be vX.Y.Z: $Version"
    exit 1
  }
}
...
function Load-LocalReleaseEnv([string]$RootPath) {
  $configured = [Environment]::GetEnvironmentVariable('DEEPSEEK_DESKTOP_RELEASE_ENV', 'Process')
  $candidates = @()
...
Write-Info "App version: $ReleaseVersion"
...
Remove-Item -Force -ErrorAction SilentlyContinue `
  (Join-Path $Root 'dist\DeepSeek-Desktop-*'), `
  (Join-Path $Root 'dist\DeepSeek Desktop-*'), `
